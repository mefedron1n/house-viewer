...
const userProjectsRoot = path.join(root, "user-projects");
...
app.post("/api/projects/:id/model", requireUser, roomUpload.single("model"), async (req, res, next) => { try { const projects = await readJsonArray(userProjectsFile), project = projects.find((item) => item.id === req.params.id && item.ownerId === req.user.id); if (!project) return res.status(404).json({ error: "Проект не найден." }); if (!req.file || path.extname(req.file.originalname).toLowerCase() !== ".glb" || req.file.buffer.subarray(0, 4).toString("ascii") !== "glTF") return res.status(400).json({ error: "Загрузите корректную GLB-модель." }); const directory = path.join(userProjectsRoot, req.user.id, project.id); await fs.mkdir(directory, { recursive: true }); await fs.writeFile(path.join(directory, "model.glb"), req.file.buffer, { mode: 0o640 }); let updated; await updateUserProjects((items) => items.map((item) => item.id === project.id ? (updated = { ...item, modelUrl: `/api/projects/${item.id}/model`, updatedAt: new Date().toISOString() }) : item)); res.status(201).json(updated); } catch (error) { next(error); } });
app.get("/api/projects/:id/model", requireUser, async (req, res, next) => { try { const project = (await readJsonArray(userProjectsFile)).find((item) => item.id === req.params.id && item.ownerId === req.user.id && item.modelUrl); if (!project) return res.status(404).end(); res.sendFile("model.glb", { root: path.join(userProjectsRoot, req.user.id, project.id) }, (error) => { if (error && !res.headersSent) res.status(404).end(); }); } catch (error) { next(error); } });
...
if (process.argv[1]?.endsWith("index.js")) fs.mkdir(roomRoot, { recursive: true }).then(() => app.listen(process.env.PORT || 3001, process.env.HOST || "0.0.0.0", () => console.log("IFC API started")));
