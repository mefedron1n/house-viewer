...
async function convert(job) {
  ...
  try {
    ...
    if (job.projectId && job.userId) {
      ...
      await updateUserProjects((projects) => projects.map((project) => project.id === job.projectId && project.ownerId === job.userId ? { ...project, modelUrl: `/api/projects/${project.id}/model`, status: "Модель готова", updatedAt: new Date().toISOString() } : project));
      Object.assign(job, { status: "ready", stage: "Модель готова", modelUrl: `/api/projects/${job.projectId}/model` });
    } else {
...
function setSessionCookie(res, token) { res.setHeader("Set-Cookie", `roomark_session=${encodeURIComponent(token)}; HttpOnly; Path=/; ${cookiePolicy()}; Max-Age=${sessionMaxAge / 1000}`); }
function clearSessionCookie(res) { res.setHeader("Set-Cookie", `roomark_session=; HttpOnly; Path=/; ${cookiePolicy()}; Max-Age=0`); }
...
async function currentUser(req) { const token = cookies(req).roomark_session; if (!token) return null; const tokenHash = crypto.createHash("sha256").update(token).digest("hex"), sessions = await readJsonArray(sessionsFile), session = sessions.find((item) => item.tokenHash.length === tokenHash.length && crypto.timingSafeEqual(Buffer.from(item.tokenHash), Buffer.from(tokenHash)) && item.expiresAt > Date.now()); if (!session) return null; return (await readJsonArray(usersFile)).find((user) => user.id === session.userId) || null; }
...
app.post("/api/auth/logout", async (req, res, next) => { try { const token = cookies(req).roomark_session; if (token) { const tokenHash = crypto.createHash("sha256").update(token).digest("hex"); await updateAuth(async () => writeJsonArray(sessionsFile, (await readJsonArray(sessionsFile)).filter((item) => item.tokenHash !== tokenHash))); } clearSessionCookie(res); res.status(204).end(); } catch (error) { next(error); } });
app.patch("/api/auth/profile", requireUser, async (req, res, next) => { try { const name = String(req.body.name || "").trim().replace(/\s+/g, " "), email = normalizeEmail(req.body.email); if (name.length < 2 || name.length > 80) return res.status(400).json({ error:"Укажите имя длиной от 2 до 80 символов." }); if (!validEmail(email)) return res.status(400).json({ error:"Укажите корректный адрес электронной почты." }); let updated; await updateAuth(async () => { const users = await readJsonArray(usersFile); if (users.some((item) => item.id !== req.user.id && item.email === email)) { const error = new Error("EMAIL_EXISTS"); error.status = 409; throw error; } await writeJsonArray(usersFile, users.map((item) => item.id === req.user.id ? (updated = { ...item, name, email, updatedAt:new Date().toISOString() }) : item)); }); res.json({ user:publicUser(updated) }); } catch(error) { if (error.message === "EMAIL_EXISTS") return res.status(409).json({ error:"Аккаунт с такой почтой уже существует." }); next(error); } });
app.patch("/api/auth/password", requireUser, async (req, res, next) => { try { const currentPassword = String(req.body.currentPassword || ""), newPassword = String(req.body.newPassword || ""); if (!await verifyPassword(currentPassword, req.user.passwordHash)) return res.status(401).json({ error:"Неверный текущий пароль." }); if (newPassword.length < 8 || newPassword.length > 128 || !/[a-zа-яё]/i.test(newPassword) || !/\d/.test(newPassword)) return res.status(400).json({ error:"Новый пароль должен содержать не менее 8 символов, букву и цифру." }); const passwordHash = await hashPassword(newPassword); await updateAuth(async () => writeJsonArray(usersFile, (await readJsonArray(usersFile)).map((item) => item.id === req.user.id ? { ...item, passwordHash, updatedAt:new Date().toISOString() } : item))); res.status(204).end(); } catch(error) { next(error); } });
...
app.delete("/api/auth/account", requireUser, async (req, res, next) => { try { const userId = req.user.id; await updateUserProjects((projects) => projects.filter((project) => project.ownerId !== userId)); await updateAuth(async () => { await writeJsonArray(usersFile, (await readJsonArray(usersFile)).filter((user) => user.id !== userId)); await writeJsonArray(sessionsFile, (await readJsonArray(sessionsFile)).filter((session) => session.userId !== userId)); }); await storage.delete(`user-projects/${userId}`, { recursive:true }).catch(() => {}); clearSessionCookie(res); res.status(204).end(); } catch(error) { next(error); } });
...
app.get("/api/models/:jobId/status", (req, res) => { const job = jobs.get(safeId(req.params.jobId)); if (!job) return res.status(404).json({ error: "Задача не найдена." }); const { id, createdAt, ownerKey, userId, ...publicJob } = job; res.json(publicJob); });
...
app.post("/api/rooms", requireUploadPassword, async (req, res, next) => {
  try {
    const name = plainLabel(req.body.name, 60), area = Number(req.body.area || 0), allowedIcons = new Set(["living-room","kitchen","kitchen-living","bedroom","nursery","office","bathroom","toilet","entryway","hallway","wardrobe","pantry","laundry","balcony","terrace"]), icon = allowedIcons.has(req.body.icon) ? req.body.icon : "living-room";
