...
test("rejects uploads when no password is configured", () => { assert.equal(validUploadPassword(""), false); assert.equal(validUploadPassword("wrong"), false); });
test("rejects malformed GLB structure and unsupported image bytes", async () => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "house-validation-")), glb = path.join(directory, "bad.glb"), ifc = path.join(directory, "bad.ifc");
  try {
    ...
    await assert.rejects(() => normalizeImage(ifc, path.join(directory, "image.png"), { maxWidth: 100, maxHeight: 100, maxPixels: 10000 }));
