...
test("registration creates a durable account and cookie session", async () => {
  ...
  try {
    ...
    const createdProjectResponse = await fetch(`${base}/api/projects`, {
      method: "POST",
      headers: { Cookie: cookie, "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Дом у сосен", area: 128, rooms: 5, theme: "Тёплый" }),
    });
    ...
    const updatedProject = await fetch(`${base}/api/projects/${createdProject.id}`, {
      method: "PATCH",
      headers: { Cookie: cookie, "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Дом у озера", area: 130, rooms: 6, theme: "Нейтральный" }),
    });
    ...
    header.writeUInt32LE(48, 8);
    header.writeUInt32LE(28, 12);
    header.writeUInt32LE(0x4e4f534a, 16);
    const modelData = new FormData();
    modelData.append(
      "model",
      new Blob([Buffer.concat([header, json])], { type: "model/gltf-binary" }),
      "model.glb"
    );
    assert.equal(
      (
        await fetch(`${base}/api/projects/${createdProject.id}/model`, {
          method: "POST",
    ...
    oversized.append(
      "model",
      new Blob([Buffer.alloc(1024 * 1024 + 1)], { type: "model/gltf-binary" }),
      "large.glb"
    );
    assert.equal(
      (
    ...
    const projects = await fetch(`${base}/api/projects`, { headers: { Cookie: cookie } }).then(
      (response) => response.json()
    );
    assert.equal(projects.length, 1);
    assert.equal("ownerId" in projects[0], false);
    const imported = await fetch(`${base}/api/account/import`, {
      method: "POST",
      headers: { Cookie: cookie, "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        version: 1,
        projects: [
          { ...createdProject, name: "Дом восстановлен" },
          { name: "Импортированный объект", area: 75, rooms: 3, theme: "Ночной" },
        ],
      }),
    });
    ...
    const restoredProjects = await fetch(`${base}/api/projects`, {
      headers: { Cookie: cookie },
    }).then((response) => response.json());
    assert.equal(restoredProjects.length, 2);
    assert.equal(
      restoredProjects.find((project) => project.id === createdProject.id).name,
      "Дом восстановлен"
    );
    assert.equal(
      restoredProjects.find((project) => project.name === "Импортированный объект").modelUrl,
      null
    );
    const importedProject = restoredProjects.find(
      (project) => project.name === "Импортированный объект"
    );
    assert.equal(
      (
        await fetch(`${base}/api/projects/${importedProject.id}`, {
          method: "DELETE",
          headers: { Cookie: cookie },
        })
      ).status,
    ...
    assert.equal(
      (
        await fetch(`${base}/api/projects`, { headers: { Cookie: cookie } }).then((response) =>
          response.json()
        )
      ).length,
      1
    ...
    assert.equal(
      (
        await fetch(`${base}/api/account/import`, {
          method: "POST",
          headers: { Cookie: cookie, "Content-Type": "application/json" },
          body: JSON.stringify({ projects: [{ name: "x" }] }),
        })
      ).status,
      400
    );
    assert.equal(
      (
        await fetch(`${base}/api/projects/${createdProject.id}`, {
          headers: { Cookie: "roomark_session=invalid" },
        })
      ).status,
      401
    );
    const expiredToken = "expired-session-token",
      sessions = JSON.parse(await fs.readFile(path.join(storage, "sessions.json"), "utf8"));
    ...
    assert.equal(
      (
        await fetch(`${base}/api/auth/me`, {
          headers: { Cookie: `roomark_session=${expiredToken}` },
        })
      ).status,
      401
    );
    ...
    const otherResponse = await fetch(`${base}/api/auth/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: "Иван Петров",
    ...
    assert.equal(
      (
        await fetch(`${base}/api/auth/login`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: "anna@example.com", password: "house2026" }),
        })
      ).status,
      200
    );
