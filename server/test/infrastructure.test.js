...
test("local storage confines keys to its root", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "house-storage-test-")),
...
test("conversion queue respects its concurrency", async () => {
  ...
  assert.equal(peak, 2);
  assert.equal(
    [...queue.jobs.values()].every((job) => job.status === "ready"),
    true
  );
});
...
test("conversion queue rejects overflow and per-owner monopolization", async () => {
  ...
  assert.throws(() => queue.enqueue({ id: "same-owner", status: "queued", ownerKey: "owner-a" }), {
    code: "OWNER_QUEUE_LIMIT",
  });
  queue.enqueue({ id: "queued", status: "queued", ownerKey: "owner-b" });
  assert.throws(() => queue.enqueue({ id: "overflow", status: "queued", ownerKey: "owner-c" }), {
    code: "QUEUE_FULL",
  });
  release();
  while (queue.running || queue.size) await new Promise((resolve) => setTimeout(resolve, 5));
...
test("conversion queue counts IFC analysis as an active owner job", () => {
  const queue = new ConversionQueue({ concurrency: 1, maxPerOwner: 1, worker: async () => {} });
  queue.jobs.set("analysis", { id: "analysis", ownerKey: "owner", status: "analyzing" });
  assert.throws(() => queue.enqueue({ id: "duplicate", ownerKey: "owner", status: "queued" }), {
    code: "OWNER_QUEUE_LIMIT",
  });
});
