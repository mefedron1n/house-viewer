...
test("room assets and notes are shared, persisted, and removable", async () => {
  ...
  await fs.writeFile(
    path.join(roomDirectory, "media.json"),
    JSON.stringify([
      {
        id: "photo-fedcba9876543210",
        filename: photoFilename,
        roomId: "kitchen",
        type: "construction",
        date: "2026-07-28",
        comment: "Установлены шкафы",
  ...
  const { deleteRoomAsset, projectManifest, readNotes, roomManifest, updateNotes } = await import(
    `../index.js?test=${Date.now()}`
  );
  assert.match(
    (await projectManifest()).floorplanUrl,
    /\/api\/project\/floorplan\/floorplan\.png\?v=/
  );
  ...
  await Promise.all(
    Array.from({ length: 10 }, (_, index) =>
      updateNotes((notes) => [
        ...notes,
        { id: String(index).padStart(24, "0"), text: `Пин ${index}` },
      ])
    )
  );
