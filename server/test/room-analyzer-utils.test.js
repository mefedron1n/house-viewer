...
import { pointInPolygon, polygonArea, simplifyPolygon } from "../../scripts/room-analyzer/geometry/polygonUtils.js";
import { assignRoomNames, classifyRoom } from "../../scripts/room-analyzer/classification/classifyRoom.js";
...
test("room polygons retain area and containment", () => {
  ...
  assert.deepEqual(simplifyPolygon([[0,0],[2,0],[4,0],[4,3],[0,3]], .01), polygon);
});
...
test("room classification recognizes combined kitchen and living room", () => {
  ...
  assert.deepEqual(rooms.map(({ name }) => name), ["Спальня", "Спальня 2", "Комната"]);
});
