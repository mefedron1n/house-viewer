...
export async function validateIfcFile(file) {
  ...
  try {
    ...
    return (
      /^ISO-10303-21\s*;/i.test(text) &&
      /HEADER\s*;/i.test(text) &&
      /FILE_SCHEMA\s*\(\s*\(\s*['"]IFC(?:2X3|4(?:X[123])?)['"]/i.test(text)
    );
  } finally {
    await handle.close();
  }
  ...
  try {
    ...
    while (offset < stat.size) {
      ...
      if (first) {
        ...
        await handle.read(json, 0, length, offset + 8);
        try {
          JSON.parse(json.toString("utf8").trimEnd());
        } catch {
          return false;
        }
...
export async function normalizeImage(input, output, limits) {
  ...
  if (
    ...
    throw Object.assign(new Error("Unsupported or oversized image"), {
      status: 400,
      code: "INVALID_IMAGE",
    });
