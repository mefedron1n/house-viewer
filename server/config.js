...
function origins() {
  ...
  for (const value of values) {
    ...
    if (!/^https?:$/.test(parsed.protocol) || parsed.origin !== value) throw new Error(`Invalid ALLOWED_ORIGINS entry: ${value}`);
  }
  return values;
}
function hosts() {
  const values = String(env.ALLOWED_HOSTS || "").split(",").map((value) => value.trim().toLowerCase()).filter(Boolean);
  if (values.some((value) => !/^[a-z0-9.-]+(?::\d{1,5})?$/.test(value))) throw new Error("ALLOWED_HOSTS contains an invalid host");
  return values;
}

const isProduction = nodeEnv === "production";
if (isProduction && !env.UPLOAD_PASSWORD) throw new Error("UPLOAD_PASSWORD is required in production");
...
if (isProduction && allowedHosts.length === 0) throw new Error("ALLOWED_HOSTS is required in production");

export const config = Object.freeze({
  nodeEnv, isProduction,
  port: integer("PORT", 3001, { min: 1, max: 65535 }), host: env.HOST || "0.0.0.0",
  allowedOrigins, allowedHosts, trustProxyHops: integer("TRUST_PROXY_HOPS", 0, { max: 10 }),
