...
Оставьте `MODEL_STORAGE_DIR=/data/models`. Материалы комнат сохраняются в `/data/models/rooms` и не удаляются по TTL, поэтому постоянный диск обязателен, если они должны переживать перезапуск или deploy. Диск привязан к одному экземпляру: не увеличивайте число инстансов этого сервиса, пока результаты лежат на локальном диске. Для масштабирования вынесите файлы в объектное хранилище (например, S3/R2) и очередь в Redis/SQS.
