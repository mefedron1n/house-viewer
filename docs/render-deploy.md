...
Оставьте `MODEL_STORAGE_DIR=/data/models`. Материалы комнат сохраняются в `/data/models/rooms`, а общие заметки — в `/data/models/site-notes.json`; они не удаляются по TTL, поэтому постоянный диск обязателен, если данные должны переживать перезапуск или deploy. Диск привязан к одному экземпляру: не увеличивайте число инстансов этого сервиса, пока результаты лежат на локальном диске. Для масштабирования вынесите файлы в объектное хранилище (например, S3/R2) и очередь в Redis/SQS.
