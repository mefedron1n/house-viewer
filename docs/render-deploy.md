...
Оставьте `MODEL_STORAGE_DIR=/data/models`. Диск привязан к одному экземпляру: не увеличивайте число инстансов этого сервиса, пока результаты лежат на локальном диске. Для масштабирования вынесите файлы в объектное хранилище (например, S3/R2) и очередь в Redis/SQS.
