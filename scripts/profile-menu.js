...
window.createProfileMenu = (container, user, apiBase) => {
  ...
  container.innerHTML = `<button class="profile-trigger" type="button" aria-expanded="false" aria-haspopup="menu"><span>${escapeHtml(firstName)}</span><strong>${escapeHtml(initials)}</strong></button><div class="profile-menu" role="menu" hidden><div class="profile-menu-user"><strong>${escapeHtml(user.name)}</strong><span>${escapeHtml(user.email)}</span></div><a role="menuitem" href="./studio.html"><span>⌂</span>Личный кабинет</a><a role="menuitem" href="./studio.html?view=projects"><span>□</span>Проекты</a><a role="menuitem" href="./studio.html?view=billing"><span>₽</span>Оплата</a><button role="menuitem" type="button" data-profile-logout><span>↪</span>Выйти из аккаунта</button></div>`;
  const trigger = container.querySelector(".profile-trigger"),
    menu = container.querySelector(".profile-menu");
  const close = () => {
    menu.hidden = true;
  ...
  container.querySelector("[data-profile-logout]").addEventListener("click", async () => {
    try {
      ...
      localStorage.removeItem("roomarkUser");
